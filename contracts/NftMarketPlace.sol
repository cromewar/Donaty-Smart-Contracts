...
contract NftMarketPlace is ReentrancyGuard, Ownable {
    ...
    // Event when a item is cancelled
    ...
    // mapping from the addresss to a Listing struct
    mapping(address => mapping(uint256 => Listing)) private s_listings;
    // mapping from and address to the number of listed items
    mapping(address => uint256) private s_proceeds;

    // Modifier to view if a nft is not listed
    ...
    // Modifier to check the nft owner.
